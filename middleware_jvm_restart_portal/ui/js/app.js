...
function resetJvmSelect(text) {
  ...
  elements.jvmSelect.innerHTML = `<option value="">${escapeHtml(text)}</option>`;
  elements.jvmSelect.disabled = true;
...
function renderJvmOptions(query = '') {
  ...
  const filtered = state.modalJvms.filter(jvm => {
    const host = jvm.host || '';
    const label = `${jvm.name} | ${host} | ${env}`.toLowerCase();
    return !normalizedQuery || label.includes(normalizedQuery);
  });
  const maxOptions = 200;
  const visible = filtered.slice(0, maxOptions);
  elements.jvmSelect.innerHTML = '<option value="">Select JVM name</option>' + visible.map(jvm => {
    const name = jvm.name;
    const host = jvm.host || '';
    const label = `${name} | ${host || '-'} | ${env.toUpperCase()}`;
    return `<option value="${escapeHtml(name)}" data-host="${escapeHtml(host)}">${escapeHtml(label)}</option>`;
  }).join('');
  elements.jvmSelect.disabled = visible.length === 0;
  ...
  if (total > maxOptions || normalizedQuery) {
    elements.jvmSearchHint.textContent = normalizedQuery
      ...
      : `Showing first ${shown} of ${total} JVMs. Use search to narrow the list.`;
...
function buildPayloadPreview() {
  const payload = { jvm_restart: { ritm_number: 'RITMTEST', env: {} } };
  state.selections.forEach(item => {
    const env = item.environment;
    const hostname = item.host || item.jvm_name;
    if (!payload.jvm_restart.env[env]) payload.jvm_restart.env[env] = { hosts: {} };
    payload.jvm_restart.env[env].hosts[hostname] = item.jvm_name;
  });
  return payload;
}
...
async function loadJvmsForEnv(env) {
  ...
  try {
    ...
    elements.jvmSearch.placeholder = 'Search by JVM name, UNIX host, or environment';
    renderJvmOptions('');
...
function wireEvents() {
  ...
  elements.jvmSearch.addEventListener('input', event => { renderJvmOptions(event.target.value); });
  elements.addForm.addEventListener('submit', event => {
    event.preventDefault();
    const env = elements.envSelect.value;
    const jvmName = elements.jvmSelect.value;
    const host = elements.jvmSelect.selectedOptions[0]?.dataset.host || jvmName;
    if (!env || !jvmName) { showModalMessage('Select both environment and JVM name.'); return; }
