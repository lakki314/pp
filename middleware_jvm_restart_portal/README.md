...
sudo cp -r app.py wsgi.py requirements.txt .envs.example data templates static scripts vendor logs /opt/middleware-jvm-restart/app/
...
```bash
sudo cp /opt/middleware-jvm-restart/app/.envs.example /opt/middleware-jvm-restart/app/.envs
sudo vi /opt/middleware-jvm-restart/app/.envs
```
...
sudo chmod 640 /opt/middleware-jvm-restart/app/.envs
...
JVMs whose `JVM_NAME` ends with `-i1` through `-i9` are excluded during ingestion. They are not shown in the searchable dropdown and cannot be submitted in the restart payload. This filter applies only to `JVM_NAME`, not `UNIX_HOST`.
...
## Troubleshooting: Controller API HTTP 406 on stdout

If the portal shows an HTTP 406 error while reading job output, it usually means the Controller stdout endpoint did not like the requested response type.

The portal requests stdout with:

```text
GET /jobs/<job_id>/stdout/?format=txt
Accept: text/plain, */*
```

This is intentional because the stdout endpoint returns plain text for `format=txt`. Normal Controller JSON endpoints still use:

```text
Accept: application/json
```

If you still see HTTP 406 after this update, verify that:

1. `CONTROLLER_API_BASE` points to the correct base, for example `https://controller.example.com/api/controller/v2` or `https://controller.example.com/api/v2`.
2. The URL keeps the trailing slash before the query string: `/stdout/?format=txt`.
3. The Controller token can view the job and its stdout.
4. A proxy or gateway is not stripping or replacing the `Accept` header.
