...
def controller_headers(profile: str = "nonprod") -> Dict[str, str]:
...
def controller_request(method: str, path: str, profile: str = "nonprod", **kwargs) -> requests.Response:
    cfg = controller_config(profile)
    response = requests.request(
        method=method,
        url=controller_url(path, profile),
        headers=controller_headers(profile),
...
def api_job_stdout(job_id: int):
    response = controller_request("GET", f"jobs/{job_id}/stdout/?format=txt", profile=get_controller_profile_for_job(job_id))
