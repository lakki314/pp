...
- Optional systemd service definition.
...
sudo mkdir -p /var/lib/file-mover/reports
...
LDAP_EMAIL_TEMPLATE={username}@example.com
```
...
LDAP_ENABLED=false
DEV_USERNAME=admin
DEV_PASSWORD=replace-this-value
...
For an unauthenticated internal SMTP relay, leave the SMTP username and password blank. Do not enable STARTTLS and implicit SSL at the same time.
...
```bash
APP_DIR=/opt/file_mover_portal
VENV_DIR=/opt/file_mover_portal/.venv
RUN_DIR=/opt/file_mover_portal/run
LOG_DIR=/var/log/file-mover/gunicorn
PID_FILE=/var/run/file-mover/file-mover.pid
...
```bash
LOG_DIR=/var/log/file-mover/gunicorn ./scripts/start.sh
```

Ensure the service account can write to overridden directories.

## Running with systemd

A sample `file-mover.service` is included. Review all paths and the service account before installation.

```bash
sudo cp file-mover.service /etc/systemd/system/file-mover.service
sudo systemctl daemon-reload
sudo systemctl enable --now file-mover.service
sudo systemctl status file-mover.service
```

When systemd manages the application, use `systemctl` rather than the standalone scripts to prevent two independent Gunicorn instances.

```bash
sudo systemctl start file-mover
sudo systemctl stop file-mover
sudo systemctl restart file-mover
sudo systemctl status file-mover
```

## Reverse proxy

Bind Gunicorn to localhost and expose the application through Nginx, Apache HTTP Server, IBM HTTP Server, or an enterprise load balancer.
...
sqlite3 /var/lib/file-mover/file_mover.db "SELECT timestamp,username,action,details FROM audit_events ORDER BY id DESC LIMIT 20;"
```
...
- Email transport encryption is mandatory when mail is enabled: either SMTP over SSL or STARTTLS.
...
sudo install -d -m 0700 -o filemover -g filemover /var/lib/file-mover/reports
...
.venv/bin/python scripts/migrate_legacy_storage.py \
  --database /var/lib/file-mover/file_mover.db \
