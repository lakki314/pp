...
class FileService:
    ...
    def list_files_page(
        ...
        sort_keys = {
            ...
            "size": lambda row: (row["size_bytes"], row["name"].casefold()),
            "modified": lambda row: (row["modified_epoch"], row["name"].casefold()),
        }
        normalized_sort = sort_by if sort_by in sort_keys else "name"
        normalized_order = sort_order if sort_order in {"asc", "desc"} else "asc"
        rows.sort(key=sort_keys[normalized_sort], reverse=normalized_order == "desc")
        total = len(rows)
