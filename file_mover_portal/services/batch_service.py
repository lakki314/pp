...
class BatchService:
    ...
    def __init__(self, store: SQLiteStore, report_dir: Path, retention_hours: int = 168, report_retention_days: int = 30) -> None:
        self.store = store
        self.report_dir = report_dir.expanduser().resolve()
        self.retention_seconds = max(1, int(retention_hours)) * 3600
        self.report_retention_seconds = max(1, int(report_retention_days)) * 86400
    ...
    def cleanup_expired(self) -> int:
        ...
        # Independently remove old Excel reports, including orphaned reports whose
        # database record was removed previously. Only the portal's batch naming
        # convention is accepted, preventing arbitrary file deletion.
        report_cutoff = time.time() - self.report_retention_seconds
        for report in self.report_dir.glob("file_move_report_*.xlsx"):
            try:
                if report.is_file() and not report.is_symlink() and report.stat().st_mtime < report_cutoff:
                    report.unlink()
                    removed += 1
