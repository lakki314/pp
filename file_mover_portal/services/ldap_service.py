...
class LDAPService:
    ...
    def __init__(self, config) -> None:
        ...
        self.display_name_attribute = config["LDAP_DISPLAY_NAME_ATTRIBUTE"]

        if not self.use_ssl:
            raise RuntimeError("LDAP authentication requires LDAPS/TLS")
        if self.required_group_dns and (
    ...
    def _lookup_identity(self, username: str) -> dict:
        ...
        try:
            with self._connection(user=self.bind_dn, password=self.bind_password) as conn:
                attributes = list(dict.fromkeys([self.group_attribute, self.email_attribute, self.display_name_attribute]))
                ...
                email_values = entry[self.email_attribute].values if self.email_attribute in entry else []
                display_name_values = (
                    entry[self.display_name_attribute].values
                    if self.display_name_attribute in entry
                    else []
                )
                return {
                    "email": str(email_values[0]).strip() if email_values else "",
                    "display_name": (
                        str(display_name_values[0]).strip()
                        if display_name_values and str(display_name_values[0]).strip()
                        else username
                    ),
                    "groups": groups,
                    "dn": entry.entry_dn,
    ...
    def _search_then_bind(self, username: str, password: str) -> dict:
        if not self.bind_dn or not self.bind_password or not self.base_dn:
